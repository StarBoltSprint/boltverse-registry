...
Grok asset manifests live in `assets/<assetId>.json` — **profiles stay separate**. **Mint only via the Grok Imagine cook rail** (automatic provenance — never a player proof step). Cook = Imagine (in the Grok chat app). Format `a_<sub>_<img|vid>_<hash8>`. Same hash = same id; first creator keeps ownership. Chests **reference** the existing `assetId` and **only** when caller `sub` === `creatorSub` (in-app: Grok chat or Build). Source = `grok-imagine` only. **HARD BAN** player file upload / X / Drive / Discord / arbitrary URL. Law: [ASSETS.md](ASSETS.md). Chest product / all-in Grok app surface / rarity / Pack receipt: [CHESTS.md](CHESTS.md).
