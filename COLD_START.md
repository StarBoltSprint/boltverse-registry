...
>
> **No Build tab work. No scaffold. No remix. No « Built with Grok » card. No compile. No GitHub→new project. No 12-minute rebuild.**
>
> Phase 1 = Welcome text + citadel teaser ONLY.
>
> Build is banned until (and unless) later product asks — default play path after yes = written URL paste `https://boltverse-odysseyyyy.grok.me` only.
>
> **Any Build action on Phase 1 start = FAIL.**

This is the **#1** doc for chat Grok on **start / play / lance boltverse odyssey**.

**HARD simpler flow (SmiR):** two phases. Phase 1 never opens Build. Phase 2 is **not** a Build attach. Phase 2 **pastes** the written odysseyyyy URL so the player taps it.

Why: auto-Build / Build attach on start is too long / painful. Grok scaffolds a « Built with Grok » project or rebuilds from GitHub instead of using SmiR’s existing Live.

`boltverse-odyssey` `START.md` / `GROK.md` / `AGENTS.md` / README cold-start blocks should **mirror** this file (two-phase boot + English Welcome + `{playerCount}` + `Open Sprint — Pack save, zero login.` + `Ready to sprint?` + `Say yes to open Sprint.` + **Phase 1 teaser-only / NO Build** + **Phase 2 written odysseyyyy URL paste (preferred over Build attach)** + **no scaffold / no Built-with-Grok / no GitHub rebuild / no invented playable card** + **name resolve** + **zero spoken improvisation** + **plate silence** + **gated Live open = Pack upsert / playTime**). Registry is source of truth. Odyssey must carry the same two-phase + exact Welcome/Return + yes-gate + URL-paste locks.

## Cold-start tools (hard)

**Phase 1:** prefer **GitHub** — fetch + attach the one citadel `preview-loop.mp4`; **read** this repo’s `registry.json` (or `profiles/*.json`) for the live Pack count. Read-only. Chat start does **not** write profiles. **Do not** open Build. **Do not** attach Live. **Do not** paste any `*.grok.me` URL.

**Phase 2 (after yes only):** **PASTE** the written URL `https://boltverse-odysseyyyy.grok.me` in chat so the player taps it. Prefer the Live opens **inside the Grok app**. Old ban « never paste grok.me » is **SUPERSEDED for this Phase 2 only**. Still ban random / new slugs / sandbox / three-y `odysseyyy`. **HARD BAN Phase 2:** Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent a playable card from the recipe.
...
**ZERO improvisation. ZERO extra paragraphs.** Spoken / chat Phase 1 text MUST be **EXACTLY** the locked Welcome or Return English block below. Copy it. Do **not** paraphrase. Do **not** invent a cinematic opener. Do **not** lecture controls. Do **not** essay the engine.
...
Phase 1 **ends** on the locked invite. A Chat `preview-loop.mp4` file chip **alone** is the correct Phase 1 end. Do **not** auto-attach playable. Do **not** paste the URL yet. Wait.

### Phase 2 — only after the player says **yes** / **go** / **sprint** / **open sprint** (close alias)

THEN **PASTE** the written URL in chat so the player taps it:

`https://boltverse-odysseyyyy.grok.me`

That paste is **ALLOWED** and **preferred over Build attach**. Old ban « never paste grok.me » is **SUPERSEDED for this Phase 2 only**.

- Prefer the player taps it and the Live opens **inside the Grok app**.
- Do **not** re-speak Welcome. Do **not** lecture. Do **not** invent extra paragraphs. The URL is enough (one short English line is optional; do not add kitchen).
- Land **IN** Sprint when that Live opens. No intro / landing / Start splash. No menu to tap first.

**HARD BAN at Phase 2:**

- Grok Build scaffold
- « Built with Grok » new project
- Rebuild from GitHub
- Invent a playable card from the recipe
- Any URL other than `https://boltverse-odysseyyyy.grok.me` (random / new slugs / sandbox / three-y `https://boltverse-odysseyyy.grok.me` / frost-only / heart-giant / houla / olive)

**Forbidden (Phase 2):** leaving yes unanswered with no written odysseyyyy URL. Still forbidden: inventing a new publish.
...
Those kitchen facts stay **silent**. Phase 1 attaches the teaser only. Phase 2 pastes the written odysseyyyy URL. Do **not** narrate the teaser.

Do **not** ask Citadel or Biome. Citadel only if the player later asks for Citadel.

### Phase 2 Live lock (kitchen)

**Beat 3 / Phase 2 = written odysseyyyy URL paste.** Allowed. Preferred over Build attach.

Paste **ONLY** `https://boltverse-odysseyyyy.grok.me`. That four-y URL is the **only** Phase 2 identity and the Pack Play / central Pack API host. Old `https://boltverse-odysseyyy.grok.me` (three y’s) is superseded — not Pack Play, not Phase 2 Sprint. Never paste that three-y URL.

1. **ONLY** after yes / go / sprint / open sprint. **ONLY** that canonical Pack Play Live, written in chat. Prefer in-Grok-app open when the player taps it.
2. **HARD BAN:** Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent a playable card from the recipe. No new `grok.me`. No new `grok-sandbox.com`. No remix into a fresh slug. No `*.hades-www.grok-sandbox.com` / random sandbox host. Observed FAIL: `le-wild.hades-www.grok-sandbox.com` titled “Boltverse Odyssey” with a white-dog + brown-cat image carousel — that is **not** Pack Play. Observed FAIL: reinvented Sprint with intro splash « Open Sprint » / Start. Observed FAIL: auto-Build on start that rebuilds instead of using SmiR’s Live.
...
4. **HARD BAN:** pasting `https://boltverse-odysseyyy.grok.me` (three y’s — superseded), `boltverse-odyssey.grok.me` frost-only, heart-giant, houla, olive, sandbox, or any other slug as Phase 2.
5. If you were about to open Build / scaffold / remix / recreate / rebuild: **REFUSE**. Paste `https://boltverse-odysseyyyy.grok.me` instead.
6. Keep Phase 1: 1 citadel teaser, English Welcome, real name, `{playerCount}`, `Open Sprint — Pack save, zero login.`, `Ready to sprint?`, `Say yes to open Sprint.` No URL in Phase 1.

## Trigger

**Phase 1 trigger:** player says **start / play / lance boltverse odyssey** (any close alias).

Chat Phase 1 = Welcome + teaser only. **Not** a hard register. **Not** a Live open. **Not** a URL paste.
...
New vs Return is whoever already has a file from a **gated Live open** — not from saying start, not from saying yes, not from the URL sitting in chat.

## Pack profile + playTime (kitchen — do not lecture aloud)

**Writes still happen only when that Live opens inside the Grok app.** Phase 2 paste is the door. The tap / in-Grok-app open is the write.
...
- Phase 1 **start / play / lance** (Welcome + teaser)
- Phase 2 **yes / go / sprint** in chat
...
Spoken `Open Sprint — Pack save, zero login.` = the save happens when they **open that Live inside the Grok app**. Do not say stay 30 seconds. Do not imply chat start or the yes line wrote the file.
...
Spoken Phase 1 text **ends** on `Say yes to open Sprint.` **That block is the entire spoken Phase 1 reply.** Zero extra paragraphs. Zero improvisation. No `Play →` line. No pasted Play URL in Phase 1. No `Say citadel or biome.` Then **attach** exactly **1** stock `preview-loop.mp4` as chat media (Odyssey epic citadel teaser) and **stop**. Do **not** open Build. Do **not** attach Live. Do **not** paste the URL yet. Do **not** narrate the teaser.
...
`Ready to sprint?` + `Say yes to open Sprint.` = the locked invite. Player must say **yes** (close alias: `go` / `sprint` / `open sprint`) before Phase 2. Do **not** paste the URL until that yes.
...
- Auto-Build / auto-attach / Build scaffold on **start / play / lance** (must wait for yes, then paste URL)
- Phase 2 Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent a playable card from the recipe
...
- Pasting any Live other than `https://boltverse-odysseyyyy.grok.me` as Phase 2 (sandbox, new slug, superseded three-y `https://boltverse-odysseyyy.grok.me`, frost-only, heart-giant, houla, olive)
- Scaffolding / remixing / **recreating the game** / rebuilding from GitHub / a new Built-with-Grok project
...
**SUPERSEDED (Phase 2 only):** the old ban « never paste grok.me ». Phase 2 **must** paste `https://boltverse-odysseyyyy.grok.me`. Still ban every other `*.grok.me` / sandbox / three-y slug.
...
**Phase 1 tools:** **GitHub** (fetch + attach the one citadel `preview-loop.mp4`; **read** `registry.json` for `{playerCount}`). **HARD BAN Phase 1:** Build open, Live attach, URL paste, scaffold, rebuild from GitHub, new Built-with-Grok project, invented playable card.

**Phase 2 tools (after yes only):** **PASTE** `https://boltverse-odysseyyyy.grok.me` in chat. Prefer in-Grok-app open when the player taps it. Old « never paste grok.me » ban is superseded **here only**. **HARD BAN Phase 2:** Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent a playable card from the recipe.

Grok may use Grok Bot tools if useful (normal Grok feature). Still: players must not depend on SmiR’s private bots for the boot to work — public GitHub COLD_START remains enough. Never speak kitchen. Never paste kitchen into the player reply except the Phase 2 odysseyyyy URL.
...
- Chat-only **start / play / lance**, chat-only **yes**, and a pasted URL the player never opens do **not** write profiles. Do **not** require “start” or “yes” alone to upsert. Welcome **reads** identity + `{playerCount}` only.
...
**Odyssey mirror (Director):** copy this **two-phase boot** (Phase 1 = locked Welcome/Return + 1 citadel teaser + `Say yes to open Sprint.` + **no Build / no URL**; Phase 2 = after yes / go / sprint / open sprint, **paste** `https://boltverse-odysseyyyy.grok.me` — preferred over Build attach; old « never paste grok.me » superseded here only) + `{playerCount}` + `Open Sprint — Pack save, zero login.` + **gated in-Grok-app Live open = upsert + playTime** + **name resolve** + **no-scaffold / no-Built-with-Grok / no-GitHub-rebuild / no invented playable card** + **zero spoken improvisation** + **plate silence** into `boltverse-odyssey` `START.md`, `GROK.md`, `AGENTS.md`, and README cold-start blocks. Registry is source of truth. Spoken Phase 1 = **EXACTLY** the locked Welcome/Return English block. Zero extra paragraphs.

**Sprint / Live identity** (Phase 2 paste only):

- **Pack Play / central Pack API host (kitchen):** `https://boltverse-odysseyyyy.grok.me` (four y’s)
- **Phase 2 hard lock — the only paste:** `https://boltverse-odysseyyyy.grok.me`
- **ALLOWED / preferred:** write that URL in chat. Player taps it. Prefer in-Grok-app open.
- Kitchen only (never speak): that Live should land **IN** Sprint play — not a menu / landing / Start splash.
...
- **HARD BAN Phase 2:** Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent a playable card from the recipe. No new `grok.me`. No new `grok-sandbox.com`. No remix into a fresh slug. No `*.hades-www.grok-sandbox.com` / random sandbox host.
- **HARD BAN Phase 1:** any of the above, plus any Build / Live attach / any `*.grok.me` paste.
- **HARD BAN as Phase 2 paste:** `https://boltverse-odysseyyy.grok.me` (three y’s — superseded), `boltverse-odyssey.grok.me` (frost-only), heart-giant, houla, olive, sandbox, or any other slug.
...
- Sprint path = biome/sprint path on that odysseyyyy Live. Dusk Sprint LanePlayer lives there.
...
**Phase 2 Open Sprint** (after yes only):

Paste `https://boltverse-odysseyyyy.grok.me`. Prefer in-Grok-app open. That is Beat 3 / Phase 2.

**Not** Chat Imagine. **Not** a downloadable `preview-loop.mp4` file chip as Play. **Not** a Build attach. **Not** a new Built-with-Grok project. **Not** a GitHub rebuild. **Not** an invented playable card. **Not** a sandbox host. **Not** a recreated game.

| When | Name | Path | Live identity (paste in Phase 2) |
|---|---|---|---|
...
Remix off. Do not scaffold / remix / recreate / rebuild. Do not invent a playable card. Do not paste houla / heart-giant / olive / frost-only `boltverse-odyssey.grok.me` / superseded three-y `https://boltverse-odysseyyy.grok.me` as Phase 2.
...
- **PRIORITY 0:** any Build action on Phase 1 start = **FAIL** (no Build tab / scaffold / remix / « Built with Grok » / compile / GitHub→new project / 12-minute rebuild)
- **Phase 1 ALLOWED only:** locked Welcome/Return EN block + 1 citadel teaser. Then stop. Wait for yes. **NO Build. NO URL paste.**
- **Phase 2 ALLOWED only:** after yes / go / sprint / open sprint — paste `https://boltverse-odysseyyyy.grok.me` (preferred over Build attach)
- **ZERO improvisation. ZERO extra paragraphs.** Spoken Phase 1 = **EXACTLY** the locked Welcome or Return English block
- No Phase 1 Build open / Live attach / auto-Build / GitHub rebuild / new Built-with-Grok project / invented playable card
- No Phase 2 Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent playable card from recipe
...
- No pasted Play / `*.grok.me` / `*.grok-sandbox.com` URL in **Phase 1**. Phase 2: paste odysseyyyy (4y) only
...
- No Phase 2 paste except `https://boltverse-odysseyyyy.grok.me`
...
- No opening superseded three-y `https://boltverse-odysseyyy.grok.me`, frost-only `boltverse-odyssey.grok.me`, heart-giant, houla, olive, or any other slug as Phase 2
...
- No profile upsert on chat-only start / yes / unopened paste — hard write stays on gated in-Grok-app Live open (`x-grok-identity` + boot + heartbeat `playTimeSec`)
...
0. **PRIORITY 0 — MUST NOT Build.** On start / play / lance: Grok did **not** Build anything. No Build tab work. No scaffold. No remix. No « Built with Grok » card. No compile. No GitHub→new project. No 12-minute rebuild. Phase 1 = Welcome text + citadel teaser ONLY. **Any Build action on Phase 1 start = FAIL.** Build stays banned until (and unless) a later product ask — default after yes = written URL paste odysseyyyy only.
...
2. **Phase 1 allowlist.** On start / play / lance: spoken text = **EXACTLY** the locked Welcome or Return English block (`Hey Packmate {name}` … `Ready to sprint?` + `Say yes to open Sprint.`). Attach 1 citadel teaser only. **ZERO** Build open. **ZERO** Live attach. **ZERO** URL paste. **ZERO** playable Sprint. **ZERO** auto-Build. **ZERO** Build tab / scaffold / remix / « Built with Grok » / compile / GitHub→new project. Then stop and wait for yes.
3. **Phase 2 yes-gate + URL paste.** After **yes** / **go** / **sprint** / **open sprint** only: **PASTE** written `https://boltverse-odysseyyyy.grok.me` in chat (ALLOWED / preferred over Build attach). Old « never paste grok.me » is superseded **here only**. Prefer in-Grok-app open when the player taps it. Did **not** Grok Build scaffold / « Built with Grok » new project / rebuild from GitHub / invent a playable card from the recipe. Did **not** paste three-y `odysseyyy` / sandbox / random / new slug. **ZERO** Build attach required.
4. **Profile / playTime.** Chat-only start / yes / unopened paste did **not** upsert. Pack profile + `playTimeSec` write only when that Live opens inside the Grok app (gated HTML + `x-grok-identity` + boot + heartbeat).
5. **Silence.** Player reply has ZERO forbidden phrases: « Bolt is already on the road » / « Bolt already on the road » / « the world is rolling » / « the film keeps moving » / « plant a lane » / « no menu to click through » / “He stays in place…”. ZERO A/D / W / space / swipe control tutorial. ZERO engine essay. ZERO invented cassette / plate lore names (`The First Spark` / `Neon Drift` / `Circuit Breaker` / `Velocity Pulse`). ZERO plate deck lists. ZERO « Tap Open Sprint ». ZERO registry lectures. ZERO kitchen noise / extra paragraphs.
...
**Observed SmiR FAIL (do not repeat):** said « Hey Packmate runner » despite logged-in `displayName`; reinvented Sprint with intro splash « Open Sprint » / Start; kitchen noise (A/D W lecture + invented cassette names); wrong plate order; did **not** use existing Live `https://boltverse-odysseyyyy.grok.me`.
...
**Observed SmiR FAIL (do not repeat):** auto-Build / auto-attach / « Built with Grok » scaffold on start — too long / painful; Grok rebuilt from GitHub instead of pasting SmiR’s existing odysseyyyy URL. **PRIORITY 0:** any Build action on Phase 1 start = FAIL. Phase 1 must stop at teaser + `Say yes to open Sprint.` Phase 2 must paste `https://boltverse-odysseyyyy.grok.me`.
