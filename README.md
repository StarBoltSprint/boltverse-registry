...
**Pack profile + playTime:** Welcome register (start / Welcome) = GitHub Action `pack-welcome` / `scripts/pack-welcome.mjs` — existence + `lastSeenAt` when `sub` known. **NOT** a `*.grok.me` Pack API. Live `/v1/pack/boot` + heartbeat = **optional later** `playTimeSec` / plate views. Welcome must succeed if Live 404. Welcome does **not** fake playTime. Never invent a `sub`. No wallet. No player API keys. **Phase 2 HOLD:** durable playTime Worker from `boltverse-pack` (not grok.me). **Pack wire is automatic — never a player ask.** Every new biome / citadel / Sprint Live ships `client/pack.js` or `BOLTVERSE_PACK_ORIGIN` → Live / later Worker + plate dealer stats. Ban « install Pack wire » as a player brief. Live floor: Odyssey `biome/docs/07-pack-live.md`. Full wire: [PACK.md](PACK.md) / [COLD_START.md](COLD_START.md).
...
Any `*.grok.me` Live auto-joins central Pack profiles. Identity = Grok gate `sub`, not the URL. **Welcome writes** go to GitHub Action `pack-welcome` — **not** a grok.me host. Live playTime writes (optional later) may hit a Live Pack API or Phase 2 `boltverse-pack` Worker (HOLD). Old `https://boltverse-odysseyyy.grok.me` (three y’s) is superseded — not Pack Play, not Beat 3 Sprint. **HARD — Pack wire is automatic:** players must NEVER tell Grok « install Pack wire / plate views / playTime ». Recipe floor: `client/pack.js` or `BOLTVERSE_PACK_ORIGIN` + plate dealer `assetId` stats. When Grok creates plates / biome / citadel, mint `assetId`s + wire stats automatically ([ASSETS.md](ASSETS.md) / Odyssey `biome/docs/07-pack-live.md` / [CHESTS.md](CHESTS.md)). Ban treating Pack wire as a manual Build chore.

**Welcome register** = start / Welcome fires GitHub Action `pack-welcome` (`gh workflow run pack-welcome.yml -f sub=…` or `repository_dispatch` type `pack-welcome`) when `sub` known (existence + lastSeen; canonical `playUrl=https://boltverse-odysseyyyy.grok.me`). **HARD: not any `*.grok.me`.** **Live open** = optional later `playTimeSec` / heartbeats / plate views (gated HTML + `x-grok-identity` + `/v1/pack/boot` + heartbeat). Welcome succeeds if Live 404. Phase 2 Worker HOLD. Pack wire is recipe floor — never a player ask. Welcome `{playerCount}` = real `registry.json` players after that upsert (drop `pack-wire-*` / `pack-doc-*` / Decode Probe / Pack probe). Odyssey `START.md` / `GROK.md` / `AGENTS.md` / README / `biome/docs/07-pack-live.md` should mirror [COLD_START.md](COLD_START.md) **same-first-reply boot** (locked Welcome + citadel teaser + paste `https://boltverse-odysseyyyy.grok.me`; no yes-wait; PRIORITY 0 no Build / no rebuild; **even on Build tab: chat-only then STOP**) + **Welcome register via GitHub Action when sub known** + automatic Pack wire + optional Live playTime + name + **exact Welcome/Return only (zero improvisation)** locks (Director sync).
...
Write profile + update `registry.json` on: Welcome register (chat start when `sub` known — GitHub Action `pack-welcome` / `scripts/pack-welcome.mjs`; existence + lastSeen; **not** `*.grok.me`) · Pack boot / heartbeat later (optional Live / Phase 2 Worker — [PACK.md](PACK.md); heartbeat = `playTimeSec`) · Keep / Hang / seal / remix publish / cosmetics change. Keyed by gate `sub` when known. Never invent a `sub`.

## Cold-start

New Grok session: load `profiles/<userId>.json` (or lookup `registry.json`) before the console teaser — New vs Return from the file that already exists. If a real `sub` is known, **fire Action `pack-welcome`** (Welcome register: existence + lastSeen; merge-patch; never wipe resonance / starCore / cosmetics / pendingChests; never invent a `sub`; **not** a grok.me Pack API). Read live Pack count for `{playerCount}` (probes excluded; include this player after a successful write / dispatch). Live open is optional later for `playTimeSec` / heartbeats. Welcome succeeds if Live 404. **First reply** = **EXACTLY** the locked Welcome or Return English block (`Ready to sprint?` may stay) + 1 citadel teaser + paste `https://boltverse-odysseyyyy.grok.me`. Then **STOP**. Zero improvisation. **No Build. No rebuild. No yes-wait. Even on the Build tab.** No Implementing Sprint / LanePlayer / Copying game assets / Configuring the build / Merging styles / SSR guards / preview from the recipe / file edits for boot. **Any Build / tooling after those three beats = FAIL.** Full Welcome copy: [COLD_START.md](COLD_START.md).
